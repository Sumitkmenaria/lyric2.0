...
function App() {
  ...
  const handleTranscribeAudio = useCallback(async () => {
    ...
    setAppState(prev => ({ 
      ...
      isLoading: true, 
      error: null,
      loaderMessage: 'Transcribing audio... This may take a few minutes.'
    }));

    try {
      const transcribedLyrics = await extractLyricsFromAudio(appState.audioFile);
      
      // Convert the structured lyrics back to raw text for display
      const rawLyricsText = transcribedLyrics.map(lyric => lyric.text).join('\n');
      
      setAppState(prev => ({
        ...prev,
        rawLyrics: rawLyricsText,
        structuredLyrics: transcribedLyrics,
      ...
      setAppState(prev => ({ 
        ...
        isLoading: false,
        loaderMessage: undefined,
        error: error instanceof Error ? error.message : 'Error transcribing audio. Please try again.'
      }));
    }
  }, [appState.audioFile]);
